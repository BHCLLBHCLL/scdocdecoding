# -*- coding: utf-8 -*-
"""Writes scdm_cases\\SUMMARY.md and refreshes the status parts of README.md from manifest.json (CPython 3)."""
import json, os, re, datetime
R = r"D:\training\caedecoder\scdm_cases"
m = json.load(open(os.path.join(R, "manifest.json"), encoding="utf-8"))
cs = m["cases"]
cats = sorted(set(c["category"] for c in cs))
rows, T = [], {"ok": 0, "mismatch": 0, "failed": 0, "not_feasible": 0}
for k in cats:
    n = {s: sum(1 for c in cs if c["category"] == k and c["status"] == s) for s in T}
    for s in T:
        T[s] += n[s]
    rows.append("| %s | %d | %d | %d | %d | %d |" % (k, n["ok"], n["mismatch"], n["failed"], n["not_feasible"], sum(n.values())))
rows.append("| **total** | **%d** | **%d** | **%d** | **%d** | **%d** |" % (T["ok"], T["mismatch"], T["failed"], T["not_feasible"], sum(T.values())))
table = "| category | ok | mismatch | failed | not_feasible | total |\n|---|---|---|---|---|---|\n" + "\n".join(rows)
size_mb = sum((c.get("size") or 0) for c in cs) / 1e6
now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M UTC+8")

summary = """# scdm_cases - corpus summary

Updated %(now)s. SpaceClaim 2019 R3 (2019.3.38912), headless RunScript, V19 API. Source of truth: `manifest.json` (field `counts` has the same numbers).

## Counts per category

%(table)s

Total scdoc volume: %(mb).0f MB (of which ~312 MB is the single case `15_mesh_reduce_001_50pct`).
...
""" % {"now": now, "table": table, "mb": size_mb}
open(os.path.join(R, "SUMMARY.md"), "w", encoding="utf-8", newline="\r\n").write(summary)

rd = open(os.path.join(R, "README.md"), encoding="utf-8").read()
rd = rd.replace("08_sheetmetal\\ (GUI-made)", "08_sheetmetal\\ (scripted)")
rd = rd.replace("  _runner\\                  (planned) shared runner + common IronPython helpers",
...
rd = rd.replace('"status": "ok|failed|gui"', '"status": "ok|mismatch|failed|not_feasible"')
i = rd.find("## GUI-only cases")
if i < 0:
    i = rd.find("## Sheet metal")
if i >= 0:
    rd = rd[:i]
st = """## Sheet metal
...
%(total)d cases in `manifest.json`: **%(ok)d ok**, %(mm)d mismatch, %(f)d failed, %(nf)d not_feasible. Per-category table, remaining failures and caveats: see `SUMMARY.md`.

%(table)s
""" % {"now": now, "total": sum(T.values()), "ok": T["ok"], "mm": T["mismatch"], "f": T["failed"], "nf": T["not_feasible"], "table": table}
rd = rd.rstrip() + "\n\n" + st
open(os.path.join(R, "README.md"), "w", encoding="utf-8", newline="\r\n").write(rd)
print("SUMMARY.md", len(summary), "README.md", len(rd)); print(table)
