...
| 波次 | 名称 | 目标 | 预估（人天） | 状态 |
| --- | --- | --- | --- | --- |
| G1 | 对齐与快赢 | 目录-处理一致 + 一天内可见效的 8 项接线 | 5~7 | 未开始 |
| G2 | P0 收尾 | 纯占位清零（恢复/组件/注释/命名选择/网格三件套等） | 15~24 | 未开始 |
| G3 | 交互基础 | 工具选项生效、边/顶点拾取、测量补全、左栏三页 | 12~19 | 未开始 |
| G4 | 草图与截面 | 草图交互模式 + 8 图元 + 完整截面模式 | 21~32 | 未开始 |
| G5 | 建模命令补全 | 生成/修复/装配选项化 + 参数页 + 脚本回放扩面 | 16~26 | 未开始 |
| G6 | 研究型（按需） | scdoc 曲面与颜色、工程图视图、共享拓扑/中面、自有渲染 | 30~50+ | 未开始 |

G1–G5 合计约 **69~108 人天（3.5~5.5 人月）**；G6 不阻塞前五波，按产品需要排期。
