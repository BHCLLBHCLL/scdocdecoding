...
| 层 | 实测状态 |
| --- | --- |
| UI 命令面 | 18 ribbon 页签（+1 backstage）/ 152 命令 / **165 live（内核不可用）** / **165 live（内核可用）**；占位 prep.small、weld.symbol、safety.tab |
| 测试 | 469 条（def test_ 扫描）；命令级测试提及 112/166（其余逐条在 tests/test_coverage_ledger.py 声明理由） |
<!-- SNAPSHOT:END -->
