...
| 层 | 实测状态 |
| --- | --- |
| UI 命令面 | 18 ribbon 页签（+1 backstage）/ 154 命令 / **167 live（内核不可用）** / **167 live（内核可用）**；占位 prep.small、weld.symbol、safety.tab |
| 测试 | 485 条（def test_ 扫描）；命令级测试提及 114/168（其余逐条在 tests/test_coverage_ledger.py 声明理由） |
<!-- SNAPSHOT:END -->
