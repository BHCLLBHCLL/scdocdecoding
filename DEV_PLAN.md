...
| 官方打开我们的文件 | 根因两层已定位 | **第一层（已修复）**：attrib 链指针错位（官方布局 t2=NEXT/t3=PREV/t4=OWNER，我们写在 t0）+ class id 需按流首现序动态分配（box 与 cyl 的 id 表不同已证实，非全局常量）。**第二层（待实现）**：**流顺序 + coedge 指针语义**——混合替换实验（官方流嵌入我们的拓扑记录，ptr 语义重绑）仍被拒；coedge 的 t4/t5/t6 三指针在 box 与 cyl 中指向模式不同（环内邻居 vs 跨环 mate），语义未定。**工具已入库**：`references/diagnostics/`（流标注器、混合二分器）、`references/golden/`（盒/L/圆柱黄金参照）、`references/make_official_ref.py`（任意形状参照生成管线）、`references/verify_open.py`（官方验证闭环，阳性对照 bodies=1）。**下一步**：按黄金参照归纳 coedge t4/t5/t6 语义（或查 ACIS 文档），实现交错发射器后以混合二分器逐类验证 |
