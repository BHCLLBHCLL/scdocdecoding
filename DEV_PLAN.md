...
| 官方打开我们的文件 | 根因已定位，归律已归纳 | **流顺序敏感已字节级证实**（expG2）。**官方遍历序已从 3 个黄金参照归纳**（`references/golden/`：盒 ref_tet、凹 L 体 ref_L、圆柱 cyl——官方 STEP→scdoc 转存）：`face i` 后跟 `face(i-1) 的 loop+surface`（延迟一拍），loop 的全部 coedge 成批发射后其 edge 才出现，vertex/curve 紧随首次引用的 edge，末面 loop/surface 延迟到尾部，wstring_attrib 跟随 owner。**下一步**：在 `_build_sab` 按此规则实现交错发射器（coedge 批次边界与 C(F5) 提前混入的细节需 1-2 轮官方启动迭代验证） |
