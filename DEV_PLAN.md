...
| M4 | 扩展 | 倒圆/抽壳/阵列/镜像、截面、修复、装配、脚本 | 对盒子倒圆并阵列；缝合开放壳体；Python 建体 | 进行中（倒圆/倒角/抽壳/阵列/镜像/干涉/缝合有内核命令；螺旋插入为真实螺旋体；修复 gaps/missing/extra/small 已接线；拔模用 3 参 MakeDraft（盒体上会拒，诚实报错）；截面为基准面显示；装配/脚本待后续） |
| M5 | 后期 | `.scdoc` 写出、分面/增材/工程图 | 写出文件可被本程序回读；官方 SCDM 互操作作为加分项 | 进行中（M5-01 原生 scdoc 写出已通：scdm/scdoc_write.py 生成 5 部件包（Content_Types/rels/document.xml/SAB 二进制），盒子/棱柱回读体积一致且报告校验 19/19；scdm/facets.py STL 进会话；增材/工程图按计划后置） |
