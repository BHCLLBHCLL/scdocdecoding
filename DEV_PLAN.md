...
| 官方打开我们的文件 | 规律已破解，发射器待实现 | **已破解/修复**：① attrib 链布局（t2=NEXT/t3=PREV/t4=OWNER）；② class id 按流首现序动态分配（box/cyl id 表不同已证实）；③ **coedge t4/t5/t6 = NEXT/PREV/PARTNER 已 100% 验证**（`references/diagnostics/coedge_semantics.py`，ref_tet 24 条全符）——**我们 writer 的 coedge 布局本已正确**；④ **EDGE 发射时机**：按 doc 序处理 edge，其全部 coedge 发射后立即发 EDGE+未发射 V/C。**剩余根因 = 流顺序**（expG2 字节级证实：交错序→通过，段序→失败）。**下一步（纯实现）**：`_build_sab` 实现交错发射器——面批次（F_i, L(i-1), S(i-1)）与 edge 完成流交织，用 `references/verify_open.py` 逐迭代验证（box→cyl→混排） |
