...
| M5 | 后期 | `.scdoc` 写出、分面/增材/工程图 | 写出文件可被本程序回读；官方 SCDM 互操作作为加分项 | 进行中（scdm/facets.py：STL(二进制/ASCII) 读写、反转法向/简化、三角形网格→OCCT 壳体并进会话；facet.convert/reverse 接线；原生 scdoc 写出与增材/工程图按计划后置） |
