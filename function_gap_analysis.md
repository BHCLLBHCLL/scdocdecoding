...
## 0.0 口径修正（2026-09-12，P3）

> 本文件的「整体 ≈91%」是**自选 13 个功能域的算术均值**，与 `docs/CODE_STATE_AUDIT_20260912.md`
> 的 12 域独立复核（**≈74%**）冲突，原因有二：
>
> 1. **域清单不完整**：本文件 13 域未涵盖官方整域——梁/焊件（官方 ~43 命令类 / 37 API 类型）、
>    标准孔/螺纹/成形特征（101 类型）、CAE 网格与分析（~90 命令类 / 70 类型）、制造/去毛刺、
>    装配配置、命名视图/参考图像、PMI/GD&T。以官方 `Commands.dll` 的 688 个 `*Command` 类
>    （剔除内部后 ≈602 用户可见）为分母，我方 130 条目录命令的**功能广度 ≈20–25%**。
> 2. **百分比无独立复算来源**：本表数值为文档转述；复核后逐域值见
>    `docs/CODE_STATE_AUDIT_20260912.md` §4（格式层 85–95% 成立，专业域集中在 L1–L2）。
>
> **建议双口径分开记分**：A 格式/互操作层（scdoc 读写 + 官方双门禁，≈95–100%，成立）；
> B 功能广度层（按官方命令面，≈20–25%）；C 深度层（除格式层外 L1–L2）。
>
> 另：本文 §2.6 / §2.8 / §2.9 / §5 的若干条目已被同仓 `docs/NYI_INVENTORY.md` 的
> TODO-2/3/4/5/8 闭环记录推翻（多弯展开、折弯槽口、图幅版式、草图求解器均已实现），
> 测试数亦已从 167 更新为 300+（`def test_` 扫描）。
>
> **R3 口径补充（2026-09-12）**：本文 §2.1「官方库 6/6 解析」指的是 **SAB tokenizer** 路径；
> **文档级**（`scdm.document.load_scdoc` -> facets）在 R3/P13 之前实测 **1/6**（仅无几何的
> samplemodel6 通过，其余 5 个因 facets 体段/常量假设过严而失败或静默 0 体）。
> P13 已修正解析语法并加回归（`tests/test_library_load.py`），现 **6/6 文档级加载**，
> 且 facets 体/面数与 SAB 侧逐项一致。此后任何「解析成功率」结论必须写明口径路径。
