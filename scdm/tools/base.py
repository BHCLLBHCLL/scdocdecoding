...
class ToolManager:
    ...
    def set_mode(self, mode_id: str, wave: str, live: bool,
                 reason: Optional[str] = None) -> None:
        if not live:
            self._status(reason
                         or f"{wave} 未实现：{mode_id}（界面已切换，内核稍后接入）")
        self.mode = mode_id

    def activate(self, cmd_id: str, name: str, wave: str, live: bool, hud: str,
                 reason: Optional[str] = None) -> None:
        ...
        if live:
            ...
            self._status(reason or
                         f"{wave} 未实现：{name} — 选项已显示，提交将在内核接入后生效")
